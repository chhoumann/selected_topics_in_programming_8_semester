...
auto sum(const Container1& c1, const Container2& c2, const Containers&... cs) {
    return sum(c1) + sum(c2, cs...);
}
...
struct summation {
    ...
    auto operator()(const Containers&... cs) const {
        return sum(cs...);
    }
};

template<typename Func, typename... Containers>
auto lazy(Func&& f, Containers&&... cs) {
    static_assert(std::is_invocable_v<Func, Containers...>, "Function is not invocable with the given argument types.");
    return [f = std::forward<Func>(f), t = std::tuple(std::forward<Containers>(cs)...)]() {
