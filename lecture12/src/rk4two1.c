...
static double y_derivative(double t, double x, double y) { return -x; }
...
static double y_next(double dt, double t, double x, double y)
{
    double k1 = dt * y_derivative(t, x, y), k2 = dt * y_derivative(t + dt / 2, x, y + k1 / 2),
           k3 = dt * y_derivative(t + dt / 2, x, y + k2 / 2), k4 = dt * y_derivative(t + dt, x, y + k3);
    return y + (k1 + 2 * k2 + 2 * k3 + k4) / 6;
}

RK4_EXPORT
void solve2_1(size_t steps, double dt, double* t, double* x, double* y)
{
    assert(steps > 0);
